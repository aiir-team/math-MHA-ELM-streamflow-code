...
def draw_predict_line_with_error(data: list, error: list, filename: str, pathsave: str, exts: list):
    ...
    ax1.set(xlabel='Testing Examples', ylabel=r'Value', title='Performance Prediction: RMSE=' + str(error[1]))
...
def draw_multiple_lines(list_lines: list, list_legends: list, list_colors: list, list_markers: list,
    ...
    lgd = ax.legend(handles, labels, loc='upper center', ncol=4, bbox_to_anchor=(0.47, 1.35))
...
def draw_predict_and_error_distribution(data: list, error: list, loss:None, filename: str, pathsave: str, exts: list):
    Path(pathsave).mkdir(parents=True, exist_ok=True)
    # Import Data
    df = DataFrame({'y_true': data[0], 'y_pred': data[1]})
    list_data = [df.loc[:, "y_true"], df.loc[:, "y_pred"]]
    ...
    # sns.set(color_codes=True)
    my_fig = plt.figure(constrained_layout=True)
    gs = my_fig.add_gridspec(nrows=1, ncols=2)

    ax1 = my_fig.add_subplot(gs[0, :1])

    ax1.hist(loss, bins=10, density=True, alpha=0.6, color='g')
    ax1.axvline(x=0, color='k', linestyle='--', label='Zero line')  # add a vertical line at zero with label
    ax1.set(xlabel='% Error', ylabel='% Instances', title=f'Error Distribution: RMSE={error[1]:.3f}')
    ax1.legend()
    # ax1.set(xlabel='Months', ylabel=r'Streamflow ($m^3/sec$)')

    ax2 = my_fig.add_subplot(gs[0:, 1:])
    sns.regplot(x="y_true", y="y_pred", data=df, ax=ax2)
    ax2.set(xlabel=r'Observed', ylabel=r'Predicted', title=f'Linear Relationship: R={error[0]:.3f}')
    # ax2.set(xlabel=r'Observed ($m^3/s$)', ylabel=r'Predicted ($m^3/s$)')
    ax2.legend(['Fit'])
